...
export default function WishlistPage() {
  ...
  const [detailedWishlistProducts, setDetailedWishlistProducts] = useState<Product[]>([]);
  ...
  const handleClearAllWishlistItems = async () => {
    try {
      if (clearWishlist) { // Check if clearWishlist from context exists
        ...
        console.warn("clearWishlist function not available in context. Performing client-side clear only for now.");
        setDetailedWishlistProducts([]);
  ...
  return (
    <div className="min-h-screen bg-gray-100"> {/* Changed background for better card visibility */}
      <Header />
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-10 gap-4">
          <div>
            ...
            <p className="text-lg text-gray-500 mt-2">
              Your collection of saved items ({detailedWishlistProducts.length} items).
            </p>
          </div>
            ...
            <Dialog open={showClearConfirmDialog} onOpenChange={setShowClearConfirmDialog}>
              ...
              <DialogContent>
                ...
                <DialogFooter>
                  ...
                  <Button variant="destructive" onClick={handleClearAllWishlistItems}>
                    Confirm & Clear All
                  </Button>
                </DialogFooter>
              </DialogContent>
        ...
        {isLoading && (
          <div className="text-center py-24"> {/* Adjusted padding */}
            ...
            <p className="text-xl text-gray-700 mt-6"> {/* Adjusted margin */}
        ...
        {!isLoading && detailedWishlistProducts.length === 0 && (
          ...
          <div className="text-center py-24 bg-white rounded-xl border border-dashed border-gray-300"> {/* Styled background and border */}
            <Heart className="mx-auto h-16 w-16 text-gray-300" /> {/* Changed icon to Heart */}
            <h2 className="mt-6 text-2xl font-semibold text-gray-800"> {/* Adjusted text size */}
              Your Wishlist is Empty
            </h2>
            <p className="text-gray-500 mt-2 text-md"> {/* Adjusted text size */}
              Looks like you haven't added anything yet. Let's change that!
            </p>
            <Link href="/" passHref>
              <Button className="mt-8 h-11 px-8 bg-red-600 text-white hover:bg-red-700 transition-colors"> {/* Styled button */}
                Start Shopping
              </Button>
            </Link>
          </div>
        ...
        {!isLoading && detailedWishlistProducts.length > 0 && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-10"> {/* Adjusted gap */}
            {detailedWishlistProducts.map((product) => (
              ...
              >
                <Link href={`/products/${product.id}`} passHref className="block">
                  <div className="overflow-hidden relative w-full h-60 sm:h-56"> {/* Adjusted height for consistency */}
                    <Image
                       src={
                        product.general.images?.[0]
                ...
                <CardContent className="p-4 flex flex-col flex-grow"> {/* Ensure CardContent takes remaining space */}
                  <p className="text-xs text-gray-500 mb-1 uppercase tracking-wider">
                    {product.general.category}
                  </p>
                  <h3 className="font-semibold text-lg text-gray-800 truncate mb-1 flex-grow" title={product.general.title}>
                    <Link href={`/products/${product.id}`} className="hover:text-red-500 transition-colors">
                  ...
                  <p className="text-xs text-gray-500 mb-4">
                    SKU: {product.inventory.sku} | Stock: {product.inventory.quantity > 0 ? `${product.inventory.quantity} available` : 'Out of Stock'}
                  </p>
                  ...
                  <div className="mt-auto"> {/* Pushes buttons to the bottom */}
                    <Button
                      ...
                      disabled={isCartLoading || isWishlistLoading || isFetchingDetails || product.inventory.quantity === 0}
                    >
                      <ShoppingCart className="mr-1 h-4 w-4" /> {product.inventory.quantity > 0 ? 'Move to Cart' : 'Out of Stock'}
                    </Button>
                  </div>
                </CardContent>
              </Card>
