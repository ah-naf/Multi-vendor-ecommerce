...
export default function WishlistPage() {
  ...
  const handleMoveToCart = async (wishlistItem: WishlistItem) => {
    ...
    if (itemInCart) {
      toast.info(
        `${wishlistItem.name} is already in your cart. You can update quantity there.`
      );
    ...
    try {
      ...
      toast.error(
        `Failed to move ${wishlistItem.name} to cart: ${error.message}`
      );
    }
  };
        ...
        {wishlistItems.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {wishlistItems.map((item) => (
              <div
                key={item.productId}
                className="bg-white rounded-xl shadow-lg overflow-hidden group transition-all duration-300 ease-in-out hover:shadow-2xl"
              >
                <Link href={`/products/${item.productId}`} passHref>
                  <div className="relative w-full h-60 bg-gray-100">
                    <Image
                      src={
                        `${getBackendBaseUrl()}${item.image}` ||
