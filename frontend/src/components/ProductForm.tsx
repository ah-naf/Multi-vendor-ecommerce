...
export interface ProductFormState extends Omit<Product, "general" | "seller"> {
  general: Omit<Product["general"], "images"> & {
    images: Array<{ id: string; url: string; name: string; file?: File }>;
  };
}
...
function normalizeDisplayImages(
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const handleGeneralChange = (
    field: keyof ProductFormState["general"],
    value: any
  ) => {
    setProductState((prev) => ({
      ...prev,
  ...
  const removeImage = (imageIdToRemove: string) => {
    ...
    setSelectedFiles((prevFiles) =>
      prevFiles.filter((file) => {
        const displayImage = productState.general.images.find(
          (img) => img.id === imageIdToRemove
        );
        return !(
          displayImage &&
          displayImage.file &&
          displayImage.file.name === file.name &&
          displayImage.url.startsWith("blob:")
        );
      })
    );
  };
  ...
  const internalOnSubmit = async (e: React.FormEvent) => {
    ...
    const productDataForJson: Omit<ProductFormState, "general"> & {
      general: Omit<ProductFormState["general"], "images"> & {
        images: string[];
      };
    } = {
      ...productState,
        ...
        images: productState.general.images
          ...
          .map((img) => {
            if (img.url.startsWith(API_BASE_URL || "http://localhost:5000")) {
              return img.url.substring(
                (API_BASE_URL || "http://localhost:5000").length
              );
            }
            return img.url;
          }),
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleGeneralChange("category", e.target.value)
                }
                required
              />
            </div>
